...
class TestApp(unittest.TestCase):
    ...
    def test_app_instance(self):
        self.assertIsInstance(app, Flask, "Application is not an instance of Flask")
    ...
    def test_getAlbums(self, mocked_get):
        ...
        mocked_albums_response.json.return_value = [{'id': 1, 'title': 'Album 1'},
                                                    {'id': 2, 'title': 'Album 2'}]
        # Symuluj odpowiedź zapytania o zdjęcia
        mocked_photos_response = MagicMock()
        mocked_photos_response.json.return_value = [{'albumId': 1, 'title': 'Photo 1'},
                                                    {'albumId': 2, 'title': 'Photo 2'}]
        # Ustaw efekt dla każdego wywołania z requests.get
        mocked_get.side_effect = [mocked_albums_response, mocked_photos_response]
    ...
    def test_getPosts(self):
        with patch('main.requests.get') as mocked_get:
            ...
            mocked_posts_response.json.return_value = [{'id': 1, 'title': 'Post 1',
                                                        'body': 'Body of post 1'},
                                                       {'id': 2, 'title': 'Post 2',
                                                        'body': 'Body of post 2'}]
            # Symuluj odpowiedź zapytania o komentarze
            mocked_comments_response = MagicMock()
            mocked_comments_response.json.return_value = [{'postId': 1, 'id': 1,
                                                           'body': 'Comment 1'},
                                                          {'postId': 1, 'id': 2,
                                                           'body': 'Comment 2'},
                                                          {'postId': 2, 'id': 3,
                                                           'body': 'Comment 3'}]
            # Ustaw efekt dla każdego wywołania z requests.get
            mocked_get.side_effect = [mocked_posts_response, mocked_comments_response]
            ...
            with app.test_request_context('/posts'):
                ...
                # Sprawdź, czy dane o komentarzach są obecne w treści odpowiedzi
    ...
    def test_getPostsWithLimit(self):
        with patch('main.requests.get') as mocked_get:
            ...
            with app.test_request_context('/posts/2/2?min_chars=0&max_chars=500'):
